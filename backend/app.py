...
# app.add_middleware(
#     CORSMiddleware,
#     allow_origins=["*", "https://ecg-iit-ju-shaon.xyz"],          # covers all origins including Netlify
#     allow_credentials=True,
#     allow_methods=["*"],
#     allow_headers=["*"],
# )

# # ── Root health check (so "/" returns 200 instead of 404) ─────────────────────
# @app.get("/")
# async def root():
#     return {"status": "HealthBridge API is running ✅", "docs": "/docs"}
...
# # ── Global ECG model reference (ResNet only) ─────────────────────────────────
# resnet_model = None
...
#         raise HTTPException(
#             status_code=503,
#             detail="ECG model not loaded yet. Please retry in a moment."
#         )
#     return resnet_model
...
# def ecg_predict_signal(signal: np.ndarray, model) -> dict:
...
#     # ── Load ResNet only (lightweight, crash-safe for Render free tier) ─────────
#     try:
#         resnet_model = download_model(
#             file_id  = "17td7VKujruuvy4aP0u06B-GCTo9yMquH",
#             filename = "resnet.keras"
#         )
#         logger.info("✅ ResNet ECG model ready")
#     except Exception as e:
...
#     try:
#         model = get_ecg_model()
#         logger.info("🤖 ResNet ECG model retrieved successfully")
#     except HTTPException:
#         raise
#     except Exception as e:
...
#         result = ecg_predict_signal(signal, model)
...
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*", "https://ecg-iit-ju-shaon.xyz"],
    allow_credentials=True,
    allow_methods=["*"],
    allow_headers=["*"],
)
...
# ── Model loader ──────────────────────────────────────────────────────────────
def download_model(file_id: str, filename: str):
    """Load model from local path (already uploaded to VPS)."""
    path = ECG_MODEL_DIR / filename
    if not path.exists():
        if file_id:
            logger.info(f"⬇️  Downloading {filename} from Google Drive …")
...
# ── Global ECG model references (3 models) ───────────────────────────────────
resnet_model      = None
...
def ecg_predict_signal_ensemble(signal: np.ndarray, models: list) -> dict:
    """Run ensemble prediction using all 3 models (ResNet + Inception + Transformer)."""
    n_segments = int(len(signal) // ECG_WINDOW_SIZE)
    if n_segments == 0:
        raise HTTPException(
            status_code=400,
    ...
    for seg_idx in range(n_segments):
        ...
        seg_probs = np.zeros(len(ECG_CLASS_NAMES))
        for m in models:
            seg_probs += m.predict(x, verbose=0)[0]
        seg_probs /= len(models)

        class_idx = int(np.argmax(seg_probs))
        all_class_probs += seg_probs
        segments_result.append({
            "seg":        seg_idx + 1,
            "start_t":    round(start / ECG_SAMPLING_RATE, 1),
            "end_t":      round(end   / ECG_SAMPLING_RATE, 1),
            "prediction": ECG_CLASS_NAMES[class_idx],
            "confidence": round(float(seg_probs[class_idx]), 4),
        })
...
async def startup_event():
    ...
    try:
        resnet_model = download_model(file_id="", filename="resnet_final.keras")
        logger.info("✅ ResNet ECG model ready")
    except Exception as e:
        logger.error("❌ Failed to load ResNet model: %s", e)

    try:
        inception_model = download_model(file_id="", filename="inception_final.keras")
        logger.info("✅ Inception ECG model ready")
    except Exception as e:
        logger.error("❌ Failed to load Inception model: %s", e)

    try:
        transformer_model = download_model(file_id="", filename="transformer_final.keras")
        logger.info("✅ Transformer ECG model ready")
    except Exception as e:
...
async def ecg_predict(
    ...
    try:
        ...
        if len(signal) < ECG_WINDOW_SIZE:
            raise HTTPException(
                status_code=400,
                detail=f"Signal too short ({len(signal)} samples). Need ≥ {ECG_WINDOW_SIZE} samples."
            )
    except HTTPException:
        raise
    except Exception as e:
    ...
    # STEP 5: Get all 3 models
    try:
        models = get_ecg_models()
        logger.info("🤖 All 3 ECG models (ResNet + Inception + Transformer) retrieved")
    except HTTPException:
        raise
    except Exception as e:
        logger.error("STEP5 error: %s\n%s", e, traceback.format_exc())
    ...
    try:
        n_segments = int(len(signal) // ECG_WINDOW_SIZE)
        logger.info(f"🔬 Running ensemble prediction on {n_segments} segment(s) …")
        result = ecg_predict_signal_ensemble(signal, models)
        logger.info(f"✅ Ensemble prediction done: top={result['top_condition']}  prob={result['top_prob']}%")
    except HTTPException:
        raise
    except Exception as e:
        logger.error("STEP6 prediction error: %s\n%s", e, traceback.format_exc())
